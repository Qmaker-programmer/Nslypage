  ...
  const sequences = [
    {
      ...
      lines: [
        { text: "", cls: "", delay: 200 },
        {
          text: "🛡️  NSLY VAULT v1.0 — ÁREA RESTRINGIDA",
          cls: "t-green",
          delay: 300,
        },
        { text: "", cls: "", delay: 150 },
        ...
        { text: "  Bienvenido, Qmaker 👋", cls: "t-accent", delay: 300 },
      ],
    },
    {
      cmd: "make build-all",
    ...
    },
    {
      ...
      lines: [
        ...
        { text: "", cls: "", delay: 100 },
        { text: "  1  github.com     qmaker", cls: "t-green", delay: 280 },
...
(function easterEgg() {
  ...
  s(
    "🛡️  NSLY VAULT",
    "font-size:30px; color:#00FFAA; font-weight:bold; font-family:monospace;",
  );
  s(
    "Password Manager TUI • GPL v2 • Open Source",
    "font-size:12px; color:#7a7a99; font-family:monospace;",
  );
  s(
    "─────────────────────────────────────────────",
  ...
  s(
    "👨‍💻  Creado por Andresuno (Qmaker)",
    "font-size:13px; color:#00CFFF; font-family:monospace;",
  );
  s(
    "☕  Con MUCHÍSIMO TÈ",
  ...
  s(
    "📦  App:     https://github.com/Qmaker-programmer/Nsly",
    "color:#00FF88; font-family:monospace; font-size:12px;",
  );
  s(
    "🌐  Landing: https://github.com/Qmaker-programmer/Nslypage",
    "color:#00FF88; font-family:monospace; font-size:12px;",
  );
  s(
    "⬇️   Builds:  https://github.com/Qmaker-programmer/Nsly/releases",
    "color:#FF00AA; font-family:monospace; font-size:12px;",
  );
  s(
    "─────────────────────────────────────────────",
    "color:#222; font-family:monospace;",
  ...
  s(
    "🔐  bcrypt cost=14 + AES-256-GCM + scrypt N=32768",
    "font-size:11px; color:#8A2BE2; font-family:monospace;",
  );
  ...
  document.addEventListener("keydown", (e) => {
    console.log("Tecla presionada:", e.key, "Posición:", pos); // Debug
    if (e.key.toLowerCase() === konami[pos].toLowerCase()) {
      pos++;
      console.log("¡Coincide! Nueva posición:", pos);
      if (pos === konami.length) {
        pos = 0;
        document.body.style.animation = "none";
        
        // Flash parpadeante que se queda
        ...
        flash.style.cssText = `
                    ...
                    animation: blink-konami 1s ease-in-out infinite;
                    pointer-events:none;
                `;
        flash.textContent = "🛡️  NSLY MODE ACTIVATED  🛡️";

        const ks = document.createElement("style");
        ks.textContent = `@keyframes blink-konami { 0%, 100%{opacity:0.7} 50%{opacity:0.3} }`;
        ...
        const rainInterval = setInterval(() => {
          ...
          // Alternar entre icon.png y tux-spinning.gif
          const useGif = Math.random() > 0.5;
          icon.src = useGif ? "tux-spinning.gif" : "icon.png";
          
          // Calcular posición en pixeles
          const randomVW = Math.random() * 100;
          const leftPx = (randomVW / 100) * window.innerWidth;
          ...
          icon.style.cssText = `
                        ...
                        cursor:grab;
                        user-select:none;
                        animation: fall-konami ${3 + Math.random() * 2}s linear forwards;
                    `;
          ...
          icon.addEventListener('mousedown', (e) => {
            ...
            startX = e.clientX;
            startY = e.clientY;
            // Obtener posición actual del icono en pantalla
            const rect = icon.getBoundingClientRect();
            initialLeft = rect.left;
            initialTop = rect.top;
            // Actualizar CSS a la posición actual
            icon.style.left = initialLeft + 'px';
            icon.style.top = initialTop + 'px';
            // Cancelar eliminación mientras haces drag
          ...
          document.addEventListener('mouseup', () => {
            if (isDragging) {
              ...
              // Lanzar el icono hacia abajo con velocidad
              icon.style.animation = `fall-konami 2s linear forwards`;
              // Programar eliminación después de soltar
              removeTimeout = setTimeout(() => icon.remove(), 5000);
            }
          });
          
          // Eliminar icono después de caer (solo si no se está arrastrando)
          removeTimeout = setTimeout(() => icon.remove(), 10000);
        }, 200);

        const fallStyle = document.createElement("style");
        fallStyle.textContent = `@keyframes fall-konami { to{transform:translateY(110vh) rotate(360deg);} }`;
        document.head.appendChild(fallStyle);

        console.log(
          "%c🎮 KONAMI CODE! ¡Sabía que eras curioso 👀",
          "font-size:16px; color:#FFD700; font-family:monospace;",
        );
      }
    } else {
      pos = 0;
